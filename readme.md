...
Users upload documents (PDFs, HTML UI templates like `checkout.html`, JSON configs, Markdown/TXT docs). The backend:
- Extracts text with format-specific parsers:
  - PyMuPDF for PDF
  - BeautifulSoup for HTML
  - Native JSON parsing for JSON files
- Cleans and splits text into overlapping semantic chunks
